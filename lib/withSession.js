...
export const withAuthServerSideProps = ({ options, callback }) => {
  return withIronSession(async (context) => {
    ...
    if (defaultOptions.isProtected && defaultOptions.roles.length) {
      const isAuth = checkUserRole(defaultOptions.roles, user?.role)
    ...
    if (!user)
      return {
        props: { user: { isLoggedIn: false }, data },
      }
